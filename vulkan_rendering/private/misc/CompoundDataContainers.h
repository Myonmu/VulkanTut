...
class aliased_unordered_map {
    // Maps weak keys to strong keys (many-to-one relationship)
    std::unordered_map<WeakKey, StrongKey> weakToStrong{};
    // Maps strong keys to values (one-to-one relationship)
    std::unordered_map<StrongKey, Val> strongToVal{};
...
public:
    ...
    Val& emplace(const StrongKey& strongKey, const WeakKey& weakKey, const Val& value) {
        // Insert into the strong-to-value map
        auto [it, inserted] = strongToVal.emplace(strongKey, value);
